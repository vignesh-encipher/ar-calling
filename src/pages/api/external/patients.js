export default async function handler(req, res) {
  ...
  try {
    console.log('=== BACKEND PATIENTS API HANDLER START ===');
    console.log('Request method:', req.method);
    console.log('Request URL:', req.url);
    console.log('Batch ID:', batchId);
    
    console.log('=== BACKEND PATIENTS API DEBUG START ===');
    console.log('Fetching patients from external API...');
    console.log('Target URL: https://ar.encipherhealth.com/ar/get/allPatients?batchId=' + batchId);
    
    console.log('About to make fetch request...');
    const response = await fetch(`https://ar.encipherhealth.com/ar/get/allPatients?batchId=${batchId}`, {
      method: 'GET',
      headers: {
    ...
    console.log('External patients API response:', data);
    console.log('=== BACKEND PATIENTS API DEBUG END ===');
    console.log('Sending response to frontend:', data);
    res.status(200).json(data);
    
  } catch (error) {
    console.error('=== BACKEND PATIENTS API ERROR ===');
    console.error('Error type:', error.constructor.name);
    console.error('Error message:', error.message);
    console.error('Error stack:', error.stack);
    console.error('Error name:', error.name);
    console.error('Error code:', error.code);
    console.error('=== BACKEND PATIENTS API ERROR END ===');
